...
## R Fundamentals
### Packages
You can install new packages using `install.packages()` function.
The installed packages are loaded with `library()`.
...
### Objects
Objects can be assigned with either `<-` or `=` operators. They have exatly the same effect, it is just the matter of style preference. 
```{r}
...
```{r}
example_df[ ,c(2, 3)]
example_df[ ,c('second_column', 'third_column')]
...
Alternatively, in `fixest` package you can use `feols()` function and passing in `vcov = "hetero"` to get HC0 (White) SE. Additionally, you can also apply the small sample correction by passing in `ssc = ssc(adj = TRUE)`.
Learn more [here](https://lrberge.github.io/fixest/articles/standard_errors.html) 
```{r}
...
## Inroduction to Tidyverse
The tidyverse is an opinionated collection of R packages designed for data science. All packages share an underlying design philosophy, grammar, and data structures.

A common feature of most tidyverse is that they take the dataframe as the first argument. 

Here is a comparison of a few common operations done in tidyverse and base R (taken from [here](https://dplyr.tidyverse.org/articles/base.html)).
Also notice that 
```{r warning=FALSE, message=FALSE}
library(tidyverse)
# Subseting dataframe
# Tidyverse
swiss_filter <- filter(swiss, Catholic < 5)
# Base R
